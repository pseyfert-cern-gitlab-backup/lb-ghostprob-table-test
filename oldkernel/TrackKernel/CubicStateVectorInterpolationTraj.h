...
namespace LHCb
{
  ...
  public: // trajectory interface
    ...
    std::unique_ptr<Trajectory> clone() const override { return std::unique_ptr<Trajectory>(new CubicStateVectorInterpolationTraj(*this)) ; }
    /// Position at location z
    Point position( double z ) const override { return Point(x(z),y(z),z) ; }
    /// First derivative of position to z
    Vector direction( double z ) const override { return Vector(tx(z),ty(z),1) ; }
    /// Second derivative of position to z
    Vector curvature( double z ) const override { return Vector(omegax(z),omegay(z),0) ; }
    /// Distance in z until the deviation from the linear
    /// approximation differs from this trajectory by a given tolerance.
    double distTo1stError( double z, double tolerance, int pathDirection = +1 ) const override;
    /// Distance in z until the deviation from the quadratic
    /// approximation differs from this trajectory by a given tolerance.
    double distTo2ndError( double z, double tolerance, int pathDirection = +1 ) const override;
    /// Create a parabolic approximation to the trajectory
    void expansion(  double z, Gaudi::XYZPoint& p, Gaudi::XYZVector& dp, Gaudi::XYZVector& ddp  ) const override;
    using ZTrajectory::arclength;
    /// Arclength between 2 z -locations (not yet imlpemented)
    double arclength(double z1, double z2) const override;
  ...
  public: // other public methods
    ...
    /// state vector
    StateVector stateVector(double z) const override;
