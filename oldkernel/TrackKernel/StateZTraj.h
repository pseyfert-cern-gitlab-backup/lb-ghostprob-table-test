...
namespace LHCb
{
  ...
  public:
    ...
    std::unique_ptr<Trajectory> clone() const override { return std::unique_ptr<Trajectory>(new StateZTraj(*this)) ; }
    ...
    Gaudi::XYZPoint position( double z ) const override { return Gaudi::XYZPoint(x(z),y(z),z) ; }

    /// First derivative of the trajectory at the approximation point
    Gaudi::XYZVector direction( double z ) const override { return Gaudi::XYZVector(tx(z),ty(z),1) ; }
    ...
    Gaudi::XYZVector curvature( double z) const override { return Gaudi::XYZVector(omegax(z),omegay(z),0) ; }
    ...
    double muEstimate( const Gaudi::XYZPoint& point) const override;
    ...
    double distTo2ndError( double arclen,
                           double tolerance,
                           int pathDirection = +1 ) const override;
    ...
    StateVector stateVector( double mu ) const override;
